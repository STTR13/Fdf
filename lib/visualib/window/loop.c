...
void			loop(t_window *w)
{
	if (!w)
		return ;
	if (w->hook.key_press)
		mlx_hook(w->win_ptr, 2, 0, w->hook.key_press, w->hook.param);
	if (w->hook.key_release)
		mlx_hook(w->win_ptr, 3, 0, w->hook.key_release, w->hook.param);
	if (w->hook.mouse_press)
		mlx_hook(w->win_ptr, 4, 0, w->hook.mouse_press, w->hook.param);
	if (w->hook.mouse_release)
		mlx_hook(w->win_ptr, 5, 0, w->hook.mouse_release, w->hook.param);
	if (w->hook.mouse_move)
		mlx_hook(w->win_ptr, 6, 0, w->hook.mouse_move, w->hook.param);
	if (w->hook.expose)
